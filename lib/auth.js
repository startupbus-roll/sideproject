...
function employee_callback (email, password, done) {
    ...
    console.log('employee callback', email, password, done);
...
function buddy_strategy (email, password, done) {
...
function authenticate_employee (options) {

    console.log('authenticate employee', JSON.stringify(options));
    options = options || {};
    _.defaults(options, { 
        successRedirect: '/',
        failureRedirect: '/login',
